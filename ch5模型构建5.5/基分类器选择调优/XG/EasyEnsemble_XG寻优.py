"""
EasyEnsemble + XGBoost 参数寻优 (Optuna)
约束: Recall >= 0.7
优化目标: PR-AUC 最大
5折交叉验证
基模型: XGBoost
...
print("=" * 72)
print(" EasyEnsemble + XGBoost 参数寻优 (Optuna)")
print("=" * 72)
...
def objective(trial):
    ...
    n_est_xgb = trial.suggest_int("n_estimators_xgb", 50, 200, step=50)
...
print(f"  XGBoost max_depth      : [3, 9]")
print(f"  XGBoost learning_rate  : [0.01, 0.5] (log-scale)")
print(f"  XGBoost n_estimators   : [50, 100, 150, 200]")
...
print(f"    XGBoost max_depth      : {best_params['max_depth']}")
print(f"    XGBoost learning_rate  : {best_params['learning_rate']:.4f}")
print(f"    XGBoost n_estimators   : {best_params['n_estimators_xgb']}")
print()
...
# 7. 保存结果
# ============================================================
output_dir = r"C:\Users\86158\Desktop\ch5模型构建5.5\基分类器选择调优\XG"
...
for t in study.trials:
    if t.value > -900:
        all_trials_data.append({
            ...
            "n_estimators_xgb": t.params.get("n_estimators_xgb", None),
...
all_path = f"{output_dir}\\EasyEnsemble_XG_GridSearch_All.csv"
...
best_row = {
    ...
    "XGBoost_max_depth": best_params["max_depth"],
    "XGBoost_learning_rate": round(best_params["learning_rate"], 4),
    "XGBoost_n_estimators": best_params["n_estimators_xgb"],
...
best_path = f"{output_dir}\\EasyEnsemble_XG寻优结果.csv"
best_df.to_csv(best_path, index=False, encoding="utf-8-sig")
print(f"最优结果已保存: {best_path}")

# 7.3 Markdown 报告
md_content = f"""# EasyEnsemble + XGBoost Optuna 寻优结果报告
...
| XGBoost learning_rate | **{best_params['learning_rate']:.4f}** |
| XGBoost n_estimators | **{best_params['n_estimators_xgb']}** |
...
if valid_trials:
    ...
    for rank, t in enumerate(top_valid, 1):
        ...
        md_content += (f"| {rank} | {t.params['n_estimators_ee']} | {t.params['max_depth']} | "
                       f"{t.params['learning_rate']:.4f} | {t.params['n_estimators_xgb']} | "
...
- 最优基分类器配置：XGBoost(max_depth={best_params['max_depth']}, learning_rate={best_params['learning_rate']:.4f}, n_estimators={best_params['n_estimators_xgb']})

---
*报告生成时间: 基于 Optuna {optuna.__version__} 自动生成*
"""

md_path = f"{output_dir}\\EasyEnsemble_XG寻优结果.md"
