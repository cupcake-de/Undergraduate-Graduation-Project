"""
EasyEnsemble + AdaBoost 参数寻优 (Optuna)
约束: Recall >= 0.7
优化目标: PR-AUC 最大
5折交叉验证
基模型: AdaBoost
...
print("=" * 72)
print(" EasyEnsemble + AdaBoost 参数寻优 (Optuna)")
print("=" * 72)
...
def objective(trial):
    ...
    n_est_ada = trial.suggest_int("n_estimators_ada", 25, 100, step=25)
    lr_ada = trial.suggest_float("learning_rate_ada", 0.01, 1.0, log=True)
    max_d = trial.suggest_int("max_depth_tree", 1, 3)
...
print(f"  AdaBoost n_estimators    : [25, 50, 75, 100]")
print(f"  AdaBoost learning_rate    : [0.01, 1.0] (log-scale)")
print(f"  DecisionTree max_depth   : [1, 3]")
...
best_ee = EasyEnsembleClassifier(
    ...
    estimator=AdaBoostClassifier(
        estimator=DecisionTreeClassifier(max_depth=best_params["max_depth_tree"]),
        n_estimators=best_params["n_estimators_ada"],
        learning_rate=best_params["learning_rate_ada"],
        random_state=42,
    ),
    sampling_strategy="auto",
    replacement=False,
...
print(f"    AdaBoost n_estimators     : {best_params['n_estimators_ada']}")
print(f"    AdaBoost learning_rate    : {best_params['learning_rate_ada']:.4f}")
print(f"    DecisionTree max_depth    : {best_params['max_depth_tree']}")
print()
print(f" 评估指标 (5折交叉验证均值):")
print(f"    Accuracy   : {best_trial.user_attrs.get('accuracy', best_trial.user_attrs.get('accuracy', 0)):.4f}")
...
# 7. 保存结果
# ============================================================
output_dir = r"C:\Users\86158\Desktop\ch5模型构建5.5\基分类器选择调优\ADA"
...
for t in study.trials:
    if t.value > -900:
        all_trials_data.append({
            ...
            "n_estimators_ada": t.params.get("n_estimators_ada", None),
            "learning_rate_ada": t.params.get("learning_rate_ada", None),
            "max_depth_tree": t.params.get("max_depth_tree", None),
...
all_path = f"{output_dir}\\EasyEnsemble_ADA_GridSearch_All.csv"
...
best_row = {
    ...
    "AdaBoost_n_estimators": best_params["n_estimators_ada"],
    "AdaBoost_learning_rate": round(best_params["learning_rate_ada"], 4),
    "DecisionTree_max_depth": best_params["max_depth_tree"],
...
# 7.3 Markdown 报告
md_content = f"""# EasyEnsemble + AdaBoost Optuna 寻优结果报告
...
| AdaBoost n_estimators | **{best_params['n_estimators_ada']}** |
| AdaBoost learning_rate | **{best_params['learning_rate_ada']:.4f}** |
| DecisionTree max_depth | **{best_params['max_depth_tree']}** |
...
if valid_trials:
    ...
    md_content += "| 排名 | EE_n_est | ADA_n_est | LR | Tree_depth | Accuracy | Precision | Recall | F1 | ROC-AUC | PR-AUC |\n"
    md_content += "|---|---|---|---|---|---|---|---|---|---|---|\n"
    for rank, t in enumerate(top_valid, 1):
        attrs = t.user_attrs
        md_content += (f"| {rank} | {t.params['n_estimators_ee']} | {t.params['n_estimators_ada']} | "
                       f"{t.params['learning_rate_ada']:.4f} | {t.params['max_depth_tree']} | "
...
- 最优基分类器配置：AdaBoost(n_estimators={best_params['n_estimators_ada']}, learning_rate={best_params['learning_rate_ada']:.4f}) + DecisionTree(max_depth={best_params['max_depth_tree']})
