"""
EasyEnsemble + LightGBM 参数寻优 (Optuna)
约束: Recall >= 0.7
优化目标: PR-AUC 最大
5折交叉验证
基模型: LightGBM
...
print("=" * 72)
print(" EasyEnsemble + LightGBM 参数寻优 (Optuna)")
print("=" * 72)
...
def objective(trial):
    ...
    n_est_lgb = trial.suggest_int("n_estimators_lgb", 50, 200, step=50)
...
print(f"  LightGBM max_depth    : [3, 9]")
print(f"  LightGBM learning_rate: [0.01, 0.5] (log-scale)")
print(f"  LightGBM n_estimators : [50, 100, 150, 200]")
...
best_ee = EasyEnsembleClassifier(
    ...
    estimator=lgb.LGBMClassifier(
        ...
        n_estimators=best_params["n_estimators_lgb"],
        random_state=42,
        verbosity=-1,
        force_col_wise=True,
    ),
...
print(f"    LightGBM max_depth     : {best_params['max_depth']}")
print(f"    LightGBM learning_rate : {best_params['learning_rate']:.4f}")
print(f"    LightGBM n_estimators  : {best_params['n_estimators_lgb']}")
print()
...
# 7. 保存结果
# ============================================================
output_dir = r"C:\Users\86158\Desktop\ch5模型构建5.5\基分类器选择调优\LGBM"
...
for t in study.trials:
    if t.value > -900:
        all_trials_data.append({
            ...
            "max_depth": t.params.get("max_depth", None),
            "learning_rate": t.params.get("learning_rate", None),
            "n_estimators_lgb": t.params.get("n_estimators_lgb", None),
...
all_path = f"{output_dir}\\EasyEnsemble_LGB_GridSearch_All.csv"
...
best_row = {
    ...
    "LightGBM_max_depth": best_params["max_depth"],
    "LightGBM_learning_rate": round(best_params["learning_rate"], 4),
    "LightGBM_n_estimators": best_params["n_estimators_lgb"],
...
best_path = f"{output_dir}\\EasyEnsemble_LGB寻优结果.csv"
best_df.to_csv(best_path, index=False, encoding="utf-8-sig")
print(f"最优结果已保存: {best_path}")

# 7.3 Markdown 报告
md_content = f"""# EasyEnsemble + LightGBM Optuna 寻优结果报告
...
| LightGBM max_depth | **{best_params['max_depth']}** |
| LightGBM learning_rate | **{best_params['learning_rate']:.4f}** |
| LightGBM n_estimators | **{best_params['n_estimators_lgb']}** |
...
if valid_trials:
    ...
    md_content += "| 排名 | EE_n_est | max_depth | LR | n_est | Accuracy | Precision | Recall | F1 | ROC-AUC | PR-AUC |\n"
    md_content += "|---|---|---|---|---|---|---|---|---|---|---|\n"
    for rank, t in enumerate(top_valid, 1):
        attrs = t.user_attrs
        md_content += (f"| {rank} | {t.params['n_estimators_ee']} | {t.params['max_depth']} | "
                       f"{t.params['learning_rate']:.4f} | {t.params['n_estimators_lgb']} | "
...
- 最优基分类器配置：LightGBM(max_depth={best_params['max_depth']}, learning_rate={best_params['learning_rate']:.4f}, n_estimators={best_params['n_estimators_lgb']})

---
*报告生成时间: 基于 Optuna {optuna.__version__} 自动生成*
"""

md_path = f"{output_dir}\\EasyEnsemble_LGB寻优结果.md"
